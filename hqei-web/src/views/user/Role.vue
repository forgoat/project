<template>
  <div class="app-container">
      ...
      <el-table-column align="center" label="用户">
        <template slot-scope="scope">
          <span v-for="user in scope.row.users">
            <span v-text="user.nickname" style="display: inline-block;vertical-align: middle;margin-left: 10px;"></span>
          </span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" @close="clearTempRole">
      <el-form class="small-space" :model="tempRole" label-position="left" label-width="100px"
        ...
        <el-form-item label="菜单&权限" required>
          <div v-for=" (menu,_index) in allPermission" :key="menu.menuName" style="width: 600px">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAllPermission() {
        //查询所有权限
        roleService.getAllPermission().then(data => {
          this.allPermission = data.result;
        })
      },
      getList() {
        //查询列表
        this.listLoading = true;
        roleService.listTable().then(data => {
      ...
      createRole() {
        ...
        //添加新角色
        roleService.addRole(this.tempRole).then(() => {
          this.getList();
      ...
      removeRole($index) {
        ...
        this.$confirm('确定删除此角色?', '提示', {
          ...
          roleService.deleteRole(role.roleId).then(() => {
            _vue.getList()
          }).catch(e => {
          })
        })
